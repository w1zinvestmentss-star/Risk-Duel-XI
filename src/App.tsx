...
const questions: Question[] = [
  ...
  {
    question: "You have been notified by a patient that they recieved the discharge records belonging to another patient. What are some steps to take?",
    answers: [
      { text: "Apologize!", points: 30 },
      { text: "Ask the patient to return the records that they recieved in error", points: 25 },
      { text: "Arrange the for them to recieve the records they need", points: 20 },
      { text: "Notify the patients impacted by the breach", points: 15 },
      { text: "Destory records given in error/ensure the other patient has the correct records", points: 10 },
    ],
  },
];

const fastDucketsQuestions: Question[] = [
  {
    ...
    answers: [
      ...
      { text: "Double-check Fax numbers/emails addresses/Information", points: 25, keywords: ["double-check", "double check", "check information", "email address", "check fax", "check email"] },
  ...
  },
  {
    ...
    answers: [
      ...
      { text: "Contact the Privacy Department", points: 30, keywords: ["Contact Privacy Department", "Contact Privacy Office", "Call Privacy"] },
...
interface MenuProps {
  onStartDuel: () => void;
  onStartFastDuckets: () => void;
}

const GameMenu = ({ onStartDuel, onStartFastDuckets }: MenuProps) => (
  <>
...
const PrivacyDuelGame = ({ onGoToMenu }: GameProps) => {
  ...
  const TEAM_KEYS = ["team1", "team2"] as const;
  ...
  function awardPoints(teamKey: 'team1' | 'team2') {
  ...
  const showAnswer = (i: number) => revealed.includes(i) || hostMode;
...
const FastDuckets = ({ onGoToMenu }: GameProps) => {
  const [fdQuestions] = useState<Question[]>(() => [...fastDucketsQuestions].sort(() => 0.5 - Math.random()).slice(0, 5));
  const [phase, setPhase] = useState("ANSWERING");
  const [timer, setTimer] = useState(90);
  const [answers, setAnswers] = useState<string[]>(Array(5).fill(""));
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [revealedData, setRevealedData] = useState<RevealedAnswerData[]>([]);
  ...
  const handleReveal = (index: number) => {
    ...
    let bestMatch = { points: 0 } as Answer;

    if (userAnswer) {
      // Tokenize the user's answer into words, ignoring punctuation
